...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield exec_1.exec(`mkdir -p ${SOURCES}`);
            yield exec_1.exec('mv', [srcArchive, SOURCES]);
            // Build rpm package
            // await exec(`rpmbuild -bb ${specFile} --define="_topdir /github/workspace/${topdir}"`)
            yield exec_1.exec(`rpmbuild -bb ${specFile} --define="_topdir /github/workspace/rpmbuild"`);
            // Set outputs
            let rpm_path = '';
            // await exec('find', [`${topdir}/RPMS`, '-type', 'f'], {
            yield exec_1.exec('find', ['rpmbuild/RPMS', '-type', 'f'], {
                listeners: {
                    stdout: (data) => {
